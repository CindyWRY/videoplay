package com.test.videoplayclientsdk.stream.packer.flv;

import android.annotation.TargetApi;
import android.media.MediaCodec;

import com.test.videoplayclientsdk.stream.packer.AnnexbHelper;
...
public class FlvPacker implements Packer, AnnexbHelper.AnnexbNaluListener {
    ...
    public void onAudioData(ByteBuffer bb, MediaCodec.BufferInfo bi) {
        ...
        int audioPacketSize = FlvPackerHelper.AUDIO_HEADER_SIZE + audio.length;
        int dataSize = audioPacketSize + FlvPackerHelper.FLV_TAG_HEADER_SIZE;
        int size = dataSize + FlvPackerHelper.PRE_SIZE;
    ...
    public void onVideo(byte[] video, boolean isKeyFrame) {
        ...
        int videoPacketSize = FlvPackerHelper.VIDEO_HEADER_SIZE + video.length;
        int dataSize = videoPacketSize + FlvPackerHelper.FLV_TAG_HEADER_SIZE;
        int size = dataSize + FlvPackerHelper.PRE_SIZE;
        ByteBuffer buffer = ByteBuffer.allocate(size);
    ...
    private void writeFlvHeader() {
        int size = FlvPackerHelper.FLV_HEAD_SIZE + FlvPackerHelper.PRE_SIZE;
    ...
    private void writeMetaData() {
        ...
        int dataSize = metaData.length + FlvPackerHelper.FLV_TAG_HEADER_SIZE;
        int size = dataSize + FlvPackerHelper.PRE_SIZE;
        ByteBuffer buffer = ByteBuffer.allocate(size);
    ...
    private void writeFirstVideoTag(byte[] sps, byte[] pps) {
        int firstPacketSize = FlvPackerHelper.VIDEO_HEADER_SIZE + FlvPackerHelper.VIDEO_SPECIFIC_CONFIG_EXTEND_SIZE + sps.length + pps.length;
        int dataSize = firstPacketSize + FlvPackerHelper.FLV_TAG_HEADER_SIZE;
        int size = dataSize + FlvPackerHelper.PRE_SIZE;
        ByteBuffer buffer = ByteBuffer.allocate(size);
    ...
    private void writeFirstAudioTag() {
        int firstAudioPacketSize = FlvPackerHelper.AUDIO_SPECIFIC_CONFIG_SIZE + FlvPackerHelper.AUDIO_HEADER_SIZE;
        int dataSize = FlvPackerHelper.FLV_TAG_HEADER_SIZE + firstAudioPacketSize;
        int size = dataSize + FlvPackerHelper.PRE_SIZE;
        ByteBuffer buffer = ByteBuffer.allocate(size);
